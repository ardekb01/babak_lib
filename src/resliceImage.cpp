   ...
   float *T)
{
   ...
   for(j = 0; j < Tny; j++)
   {
      ...
      for(i = 0; i < Tnx; i++)
      {
         ...
         trg[q++] = (short)(linearInterpolator(
            x, y, obj, Onx, Ony) + 0.5);
      }
   }
   ...
   for(k = 0; k < nz2; k++)
   {
      ...
      for(j = 0; j < ny2; j++)
      {
         ...
         for(i = 0; i < nx2; i++)
         {
            ...
            {
               im2[q++] = (short)(linearInterpolator(
                  x, y, z,
                  im1,
                  nx1, ny1, nz1,
                  np1) + 0.5f);
            ...
            {
               im2[q++] = (short)(cubicSplineSynthesis(
                  c,
                  nx1, ny1, nz1,
                  x, y, z,
                  beta,
...
// warning: T will be altered.
unsigned char *resliceImage(unsigned char *im1,
   int nx1, int ny1, int nz1,
   float dx1, float dy1, float dz1,
   int nx2, int ny2, int nz2,
   float dx2, float dy2, float dz2,
   float *T)
{
   ...
   im2 = (unsigned char *)calloc(nv2, sizeof(unsigned char));
   ...
   for(k = 0; k < nz2; k++)
   {
      ...
      for(j = 0; j < ny2; j++)
      {
         ...
         for(i = 0; i < nx2; i++)
         {
            ...
            im2[q++] = (unsigned char)(linearInterpolator(
               x, y, z,
               im1,
               nx1, ny1, nz1,
               np1) + 0.5f);
...
float *resliceImage(float *im1, DIM dim1, DIM dim2, float *T, int interpolation_method)
{
   ...
   xc2 = dim2.dx * (dim2.nx - 1) / 2.0f;
   yc2 = dim2.dy * (dim2.ny - 1) / 2.0f;
   zc2 = dim2.dz * (dim2.nz - 1) / 2.0f;

   np1 = dim1.nx * dim1.ny;

   xc1 = dim1.dx * (dim1.nx - 1) / 2.0f;
   yc1 = dim1.dy * (dim1.ny - 1) / 2.0f;
   zc1 = dim1.dz * (dim1.nz - 1) / 2.0f;
         ...
         for(i = 0; i < dim2.nx; i++)
         {
            ...
            if(interpolation_method == LIN)
            {
               im2[q++] = linearInterpolator(
                  x, y, z,
                  im1,
                  dim1.nx, dim1.ny, dim1.nz,
                  np1);
            }
            else if(interpolation_method == NEARN)
            {
               im2[q++] = nearestNeighbor(
                  x, y, z,
                  im1,
                  dim1.nx, dim1.ny, dim1.nz,
                  np1);
...
void resliceImage(SHORTIM im1, SHORTIM &im2, float *T, int interpolation_method)
{
   ...
   xc2 = im2.dx * (im2.nx - 1) / 2.0f;
   yc2 = im2.dy * (im2.ny - 1) / 2.0f;
   zc2 = im2.dz * (im2.nz - 1) / 2.0f;

   xc1 = im1.dx * (im1.nx - 1) / 2.0f;
   yc1 = im1.dy * (im1.ny - 1) / 2.0f;
   zc1 = im1.dz * (im1.nz - 1) / 2.0f;
   ...
   for(k = 0; k < im2.nz; k++)
   {
      ...
      for(j = 0; j < im2.ny; j++)
      {
         ...
         for(i = 0; i < im2.nx; i++)
         {
            ...
            if(interpolation_method == LIN)
            {
               im2.v[q++] = (short)(linearInterpolator(
                  x, y, z,
                  im1.v,
                  im1.nx, im1.ny, im1.nz,
                  im1.np) + 0.5f);
            }
            else if(interpolation_method == NEARN)
            {
               im2.v[q++] = (short)(nearestNeighbor(
                  x, y, z,
                  im1.v,
                  im1.nx, im1.ny, im1.nz,
                  im1.np) + 0.5f);
...
float linearInterpolator(float x, float y, float z, float *array,
                         int nx, int ny, int nz, int np, float *w)
{
   int i, j, k, n;
   float u, uu;
   float v, vv;
...
unsigned char linearInterpolator(float x, float y, float z,
                                unsigned char *array,
...
short *computeReslicedImage(short *im1,
   int nx1, int ny1, int nz1,
   float dx1, float dy1, float dz1,
   int nx2, int ny2, int nz2,
   float dx2, float dy2, float dz2,
   ...
   for(int k = 0; k < nz2; k++)
      for(int j = 0; j < ny2; j++)
         for(int i = 0; i < nx2; i++)
         {
            ...
            im2[q++] = (short)(linearInterpolator(
               x, y, z, im1, nx1, ny1, nz1, np1) + 0.5);
         }

   return im2;
}

float *computeReslicedImage(float *im1,
   int nx1, int ny1, int nz1,
   float dx1, float dy1, float dz1,
   int nx2, int ny2, int nz2,
   float dx2, float dy2, float dz2,
   ...
   float *Xwarp, float *Ywarp)
{
   ...
   for(int j = 0; j < ny2; j++)
   {
      for(int i = 0; i < nx2; i++)
      {
         ...
         im2[q++] = (short)(linearInterpolator(x, y,
            im1, nx1, ny1) + 0.5);
      }
   }
...
short *computeReslicedImage(float *im1,
   int nx1, int ny1,
   float dx1, float dy1,
   int nx2, int ny2,
   float dx2, float dy2,
...
unsigned char PNN(float x, float y, float z,
   unsigned char *array,
...
float partial_var(float x, float y, float z,
   unsigned char *array,
   int nx, int ny, int nz, int np,
   float mu)
{
   ...
   for(k = 0; k < nz2; k++)
   {
      ...
      for(j = 0; j < ny2; j++)
      {
         ...
         for(i = 0; i < nx2; i++)
         {
            ...
            im2[q] = linearInterpolator(x, y, z,
               im1, nx1, ny1, nz1, np1);
