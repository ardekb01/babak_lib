...
void symmetric_registration(SHORTIM &aimpil, const char *bfile, const char *ffile, const char *blmfile,const char *flmfile, int verbose)
{
   ...
   if(get_nifti_filename(bprefix, sizeof(bprefix), bfile) == false)
   {
      exit(0);
   }
   ...
   if(get_nifti_filename(fprefix, sizeof(fprefix), ffile) == false)
   {
      exit(0);
   }
...
void compute_hi(char *imfile, char *roifile, float4 &parenchymasize, int &voisize)
{
   ...
   if(get_nifti_filename(roifileprefix, sizeof(roifileprefix), roifile) == false) exit(1);
...
int main(int argc, char **argv)
{
   ...
   if( check_nifti1_magic(bfile)  == true )  // a single image was specified using -i <image>.nii
   {
      ...
      if(get_nifti_filename(imagefileprefix[0], sizeof(imagefileprefix[0]), imagefile[0]) == false) { exit(0); }
      scalefactor[0]=1.0;
   ...
   else
   {
      ...
      for(int i=0; i<nim; i++)
      {
         ...
         if( get_nifti_filename(imagefileprefix[i], sizeof(imagefileprefix[i]), imagefile[i]) == false) { exit(0); }
